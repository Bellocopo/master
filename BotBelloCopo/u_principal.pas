...
type
  TfrmPrincipal = class(TForm)
    ...
    function EnviaPesquisa(ANumeroCelular:String; ANomeContato:String): Boolean;
...
uses
  Vcl.Imaging.jpeg, Datasnap.DBClient, Winapi.ShellAPI, rotinas, dmAPI;
...
function TfrmPrincipal.EnviaPesquisa(ANumeroCelular:String; ANomeContato:String): Boolean;
var
 LMensagem : String;
begin
  result := false;
  try
    if not TInject1.Auth then
      ...
      LMensagem :=
          TInject1.Emoticons.Sorridente+ 'Olá *'+ANomeContato+'!*\n\n'+
          '*Pesquisa de satisfação Bellocopo*!\n\n'+
          'De 1 a 10, qual a chance de você recomendar a Bellocopo ?\n\n';

      TInject1.SendFile(ANumeroCelular, ExtractFileDir(Application.ExeName)+'\Img\bellocopo.jpg', ANumeroCelular);
      result :=true;
  finally
  end;
end;
...
begin
  try
    ...
    api.ObterPedidosWA;
    if not api.pedidoWA.IsEmpty then
    begin
      api.PedidoWA.First;
      while api.pedidoWA.Eof do
      begin
       if EnviaPesquisa(
         api.pedidoWA.FieldByName('Celular_Cliente__c').AsString,
         api.pedidoWA.FieldByName('NomeCliente__c').AsString
         ) then
         begin
           api.PatchDataSF
              (
                'https://bellocopo.my.salesforce.com/services/data/v20.0/sobjects/order/'+api.pedidoWA.FieldByName('id').AsString,
                api.JSONChequeSF
              )
         end;
      end;
...
begin
  ...
  { if ( POS('OLA', AnsiUpperCase(pMensagem))        > 0 ) or ( POS('OLÁ', AnsiUpperCase(pMensagem))       > 0 ) or
      ...
      begin
        mensagem :=
          TInject1.Emoticons.Sorridente+ 'Olá *'+pContato+'!*\n\n'+
          '*Pesquisa de satisfação Bellocopo*!\n\n'+
          'De 1 a 10, qual a chance e recomendar a Bellocopo ?\n\n';
        TInject1.SendFile(pTelefone, ExtractFileDir(Application.ExeName)+'\Img\bellocopo.jpg', mensagem);     // SendImage
        Result := True;
        exit;
      end;

      if ( POS('2', pMensagem) > 0 ) then
      begin
        mensagem :=
        TInject1.Emoticons.Telefone+ 'Telefone: (11)4529-3790\n'+
        'Site: www.bellocopo.com\n'+
        'Facebook: https://www.facebook.com/bellocopo\n';
        TInject1.Send(pTelefone, mensagem);    // Send somente texto
        Result := True;
        exit;
      end;

      if ( POS('4', pMensagem) > 0 ) then
      begin
        mensagem :=
        'Horarios de funcionamento: Seg a sex das 08:00 as 18:00\n';
        TInject1.Send(pTelefone, mensagem);
        Result := True;
        exit;
      end;

      if ( POS('5', pMensagem) > 0 ) then
      begin
        mensagem :=
        TInject1.Emoticons.AtendenteH+ 'Olá *'+pContato+'!*\n\n'+
        'Você está no auto atendimento da *Bellocopo*!\n\n'+
        'Digite um número:\n\n'+
        TInject1.Emoticons.Um             +' Suporte\n\n'+
        TInject1.Emoticons.Dois           +' Contato\n\n'+
        TInject1.Emoticons.Tres           +' Financeiro\n\n'+
        TInject1.Emoticons.Quatro         +' Horários de atendimento\n\n'+
        TInject1.Emoticons.Cinco         +' Recomeçar\n\n';
        TInject1.SendFile(pTelefone, ExtractFileDir(Application.ExeName)+'\Img\bellocopo.jpg', mensagem);
        Result := True;
        exit;
      end; }


    LNota := SomenteNumeros(pMensagem);
    if TryStrToInt(LNota,INota) then
    begin
      if (INota>=1) or (INota<=10) then
        mensagem :=
          'Obrigado pela sua resposta '+TInject1.Emoticons.Sorridente+TInject1.Emoticons.Like
       else
        mensagem :=
          'Favor, informar um número entre 1 e 10 '+TInject1.Emoticons.Like;
    end;
    TInject1.Send(pTelefone, mensagem);    // Send somente texto
    Result := True;
end;
